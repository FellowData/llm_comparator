...
def render_history_tab(evaluator=None):
    ...
    if history_data:
        ...
        # === ANALYSE BATCH HISTORIQUE VIA SERVICE ===
        if has_evaluation_data and len(df) > 5:  # Au moins 6 entrées pour l'analyse
            st.markdown("---")
            st.markdown("## 🔬 Analyse Batch Historique (alpha version)")
            ...
            if st.button("🚀 Générer Analyse Comparative Historique"):
                ...
                with st.spinner("Analyse en cours des données historiques..."):
                    # Utiliser le service d'évaluation pour l'analyse historique
                    evaluation_service = get_evaluation_service()
                    
                    # Afficher le dashboard historique
                    st.markdown("### 📈 Rapport Historique Complet")
                    evaluation_service.display_historical_dashboard(history_data)
                    
                    # Insights spécifiques à l'historique (garder le code existant)
